package nyit.apl.KnightsTour;  // CC we have create new package with different name , under that create a project with name knightsTour. 
...
public abstract class ChessBoard extends JFrame
{
	 ...
	 */
	protected static int knightColumn;
	protected static int knightRow;

	/**
	 * the size of the board is set to eight a value lesser than 8 can be used
	 * but not greater
	 */
	static private int boardSize=5;
	static public int[][] mark = new int[boardSize][boardSize];
	/**
	...
	protected static final long SLOW_DELAY = 200;
		...
		layout.setHorizontalGroup(layout
				...
				.addGroup(
						layout.createParallelGroup(
								...
								.addComponent(performanceCounterLabel, 40, 50,
										100)));
		...
		layout.setVerticalGroup(layout
				...
				.addGroup(
						layout.createSequentialGroup()
								...
								.addComponent(performanceCounterLabel, 10, 15,
										20).addContainerGap()));
	...
	private JButton getPlayButton()
	{
		if (playButton == null)
		{
			...
			System.out.println(playButton.getSize().width + playButton.getSize().height);
	...
	{
		for(int i=0;i<boardSize;i++)			// Initializing visited squares
		...
		if (jComboBoxSize == null)
		{
			...
			jComboBoxSize.addItemListener(new ItemListener()
			{
				...
				public void itemStateChanged(ItemEvent arg0)
				{
					...
					updatePerformanceCounter(0);
					mark = new int[boardSize][boardSize];
					resetMark();
				}
			});
		}
		...
		Thread tourThread = new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				for (int i = 0; i < path.size(); i++)
				{
					...
					drawKnight(nextPos);
					currentPos = nextPos;
					updatePerformanceCounter(i+1);			// Updates the number of steps taken
															// till now
					delay(delayTime);
				}
	//			updatePerformanceCounter(movesCounter);
...
// The below part helps us to place the knight
	...
	protected void updatePerformanceCounter(int moves)
	{
		if (moves < 0)
		{
		...
		{
			performanceCounterLabel.setText("Steps: " + moves);
		}
